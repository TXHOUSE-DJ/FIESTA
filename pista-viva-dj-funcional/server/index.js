...
function rateLimit(record, clientId, action, ms) {
  ...
  if (current - previous < ms) throw httpError('Esperá unos segundos y volvé a intentar.', 429);
...
io.on('connection', (socket) => {
  ...
  socket.on('song:create', (payload, ack) => {
    try {
      ...
      rateLimit(record, clientId, 'song:create', 8000);
  ...
  socket.on('song:vote', (payload, ack) => {
    try {
      ...
      requirePublicOpen(state);
      const clientId = text(payload.clientId || socket.data.clientId, 80);
      rateLimit(record, clientId, 'song:vote', 800);
  ...
  socket.on('mood:vote', (payload, ack) => {
    try {
      ...
      requirePublicOpen(state);
      const clientId = text(payload.clientId || socket.data.clientId, 80);
      rateLimit(record, clientId, 'mood:vote', 700);
  ...
  socket.on('dedication:create', (payload, ack) => {
    try {
      ...
      requirePublicOpen(state);
      const clientId = text(payload.clientId || socket.data.clientId, 80);
      rateLimit(record, clientId, 'dedication:create', 10000);
  ...
  socket.on('energy:add', (payload, ack) => {
    try {
      ...
      requirePublicOpen(state);
      const clientId = text(payload.clientId || socket.data.clientId, 80);
      rateLimit(record, clientId, 'energy:add', 250);
  ...
  socket.on('battle:vote', (payload, ack) => {
    try {
      ...
      requirePublicOpen(state);
      const clientId = text(payload.clientId || socket.data.clientId, 80);
      rateLimit(record, clientId, 'battle:vote', 700);
  ...
  socket.on('trivia:vote', (payload, ack) => {
    try {
      ...
      requirePublicOpen(state);
      const clientId = text(payload.clientId || socket.data.clientId, 80);
      rateLimit(record, clientId, 'trivia:vote', 700);
