...
function getDashboardUrl(eventId) {
  return `${window.location.origin}/show/${encodeURIComponent(eventId || '')}?dashboard=1`;
}
    ...
    <div className={cls('glow-border rounded-[28px] border border-neonCyan/30 bg-neonCyan/10 p-4 shadow-neonCyan', compact && 'p-3')}>
      <div className="flex items-start gap-4">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-2 break-all text-[11px] text-softText">{getDashboardUrl()}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="app-shell">
      <div className="app-frame px-4 py-[calc(env(safe-area-inset-top)+24px)]">
        <Brand />
        <Panel className="glow-border mt-8">
...
function ShowApp({ pista }) {
  ...
  const qrUrl = state.event.publicUrl || `${window.location.origin}/e/${state.event.id}`;
  const forceDashboard = new URLSearchParams(window.location.search).get('dashboard') === '1';
  const mode = forceDashboard ? 'live' : state.screen.mode;

  return (
    <div className="show-frame">
      <ScreenAnnouncementBanner state={state} />
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col p-5 sm:p-8">
        ...
        <main className="grid flex-1 place-items-center py-6">
          {mode === 'live' && <ShowLiveDashboard state={state} connected={connected} qrUrl={qrUrl} />}
...
function ShowLiveDashboard({ state, connected, qrUrl }) {
  ...
  return (
    <div className={cls('live-dashboard w-full', getEnergyMode(state.energy.score) !== 'normal' && 'live-dashboard-boost')}>
      <EnergyOverdriveOverlay score={state.energy.score} />
      <div className="mb-5 grid items-center gap-4 lg:grid-cols-[1fr_auto]">
        <div>
